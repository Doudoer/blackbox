    ...
    <div className="msg-item" style={{ display: 'flex', justifyContent: isOwn ? 'flex-end' : 'flex-start', width: '100%' }}>
      <div className={`bubble ${isOwn ? 'sent' : 'recv'} ${enter ? 'msg-enter' : ''}`}>
        ...
        {message.message_type === 'sticker' && (message.sticker_url) && (
          // If sticker_url looks like an URL, render image, otherwise render large emoji text
          (typeof message.sticker_url === 'string' && (message.sticker_url.startsWith('http') || message.sticker_url.startsWith('data:')))
            ? // eslint-disable-next-line @next/next/no-img-element
              <img src={message.sticker_url} alt="sticker" style={{ borderRadius: 12, maxHeight: 220, objectFit: 'contain' }} />
            : <div style={{ fontSize: 48, lineHeight: 1 }}>{message.sticker_url}</div>
        )}
