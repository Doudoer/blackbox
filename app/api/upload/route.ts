...
export async function POST(req: Request) {
  try {
    ...
    const bucket = process.env.NEXT_PUBLIC_SUPABASE_STORAGE_BUCKET || 'blackboxbucket'

    // build a path including the user id for easy ownership lookup
    // attempt to determine a file extension from content-type
    let ext = ''
    const m = contentType.match(/image\/(png|jpeg|jpg|gif|webp)/)
    if (m) {
